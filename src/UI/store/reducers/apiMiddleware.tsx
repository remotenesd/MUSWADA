    ...
    return function(next){
        return function(action)
        {
            ...
            if (action.type === ActionTypes.REGISTER)
            {
                ...
                //!important
                action.payload.user.password = CryptoJS.SHA256(action.payload.user.password).toString(CryptoJS.enc.Hex);
                
                Axios.post(globals.baseURL + '/' + globals.userURL + '/register', action.payload.user).then ( res => {
                    console.log(res.status);
                    if (res.status == 200)
                    {   
                        action.success = true;
                    }else {action.success = false;}
                    // redirect to register successful page
                    const nextAction = next(action);
                    //read next state
                    const state = getState();
            ...
            if (action.type === ActionTypes.USERNAME_EXIST)
            {
                ...
                Axios.post(globals.baseURL + '/' + globals.userURL + '/isUsernameTaken',{ username : action.payload.username}).then ( res => {
