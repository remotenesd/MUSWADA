...
import { filterMecanicienParFonction, fonctions, grades } from '../store/core/core';
...
const Person = (
    ...
    const mapItems = () => {
        const keys = Object.keys(grades).reverse()
        return keys.map(grade => (<option>{grade}</option>))
    }
    
    const mapSpec = () => {
        const keys = Object.entries(fonctions).reverse().filter(e => e[0].match(/[a-zA-Z]/));
        return keys.map(grade => (<option>{grade[0]}</option>))
    }
            ...
            <div className="maindPers align-items-center justify-content-center overflow-scroll" style={{paddingTop: '20px'}}>
                ...
                <Form className=' my-2 py-1 ' style={{height: '70vh', overflowX: 'hidden', overflowY: 'scroll', marginBottom: '50px'}}>
                        ...
                        <FormGroup row hidden={!validateMatricule()['result']}>
                            <Label size="lg" sm={4} for="fonction">Specialite :</Label>
                            <Col>
                                {/* <Input sm={8} bsSize="lg"
                                    type="text"
                                    name="fonction"
                        ...
                        {
                            validateFonction()['result'] 
                            ?
                            (
                                filterMecanicienParFonction(fonctions[fonction]) 
                                ?
                                <Label>
                                    Specialite : Mecanicien / {fonction}
                                </Label> 
                                :
                                <Label>
                                    Specialite : Pontiste / {fonction}
                                </Label>
                            )
                            :
                            <></>
