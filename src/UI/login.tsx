...
const Login = ({ session, meSetLogin }) => {
  ...
  return (
    <div className="keepSmall">
      <Form>
        <FormGroup row>
          <Label size="lg" xs={12} sm={4} for="ide">
            <b>Identifier</b>
          </Label>
          <Col>
            <Input
        ...
        <FormGroup row hidden={identifier.length < 3}>
          <Label size="lg" sm={4} for="password">
            Pass Phrase
          </Label>
          <Col>
            <Input
        ...
        <Link to="/register" className="fixed-bottom m-2 p-1">
          No account ? Join us now.
        </Link>{" "}
        <br />
        <Button size="lg" onClick={sendData} color="primary">
          SIGN IN
        </Button>
      </Form>
      <br />
      <Link
        to="/"
        className="biggerText fixed-bottom m-2 my-5  p-1 float-right"
      >
        [-] Go back home.
      </Link>
    </div>
